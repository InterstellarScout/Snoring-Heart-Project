...
DUAL_BUTTON_ENABLED = True
# M5's schematic and example indicate active-low; physical verification on the
# installed U025 is still required before acceptance testing is complete.
BUTTON_ACTIVE_LEVEL = False
BUTTON_POLARITY_PHYSICALLY_VERIFIED = True
BUTTON_DEBOUNCE_MS = 40
DOUBLE_CLICK_WINDOW_MS = 350
BUTTON_CHORD_WINDOW_MS = 120
BUTTON_LONG_HOLD_MS = 3000
MASTER_STEP = 5
...
BREATHING_MOTOR_MODES = ("both", "motor1", "motor2", "alternating")
SCD43_ADDRESS = 0x62
...
def breathing_envelope(elapsed, interval):
    """Smooth zero-to-peak-to-zero envelope for one complete breath."""
    if interval <= 0:
        return 0.0
    progress = clamp(elapsed / interval, 0.0, 1.0)
    ramp = progress * 2.0 if progress <= 0.5 else (1.0 - progress) * 2.0
...
class BreathingScheduler:
    """Continuous breathing envelope with settings adopted at cycle boundaries."""
    ...
    def __init__(self, haptics, state):
        ...
        self.deadline = 0
        self.update_at = 0
        self.interval = 60.0 / BREATH_RATE_DEFAULT_BPM
        self.strength = 0
        self.motor_mode = "both"
        self.direction = 0
        self.next_direction = 0
    ...
    def reset(self, now, preserve_output=False):
        ...
        self.started = 0
        self.update_at = now
        self.deadline = now + 60.0 / self.state_values["breath_rate"]
    ...
    def snapshot_cycle(self):
        self.interval = 60.0 / self.state_values["breath_rate"]
        self.strength = scaled_strength(
            self.state_values["breath_intensity"],
            self.state_values["master_power"],
        )
        self.motor_mode = self.state_values["breath_motor_mode"]
        self.direction = self.next_direction
    ...
    def update(self, now):
        ...
        if now >= self.deadline:
            elapsed = now - self.started
            completed = max(1, int(elapsed / self.interval))
            self.started += completed * self.interval
            if self.motor_mode == "alternating" and completed % 2:
                self.next_direction = 1 - self.next_direction
            self.snapshot_cycle()
        ...
        if now >= self.update_at:
            strength = int(
                self.strength
                * breathing_envelope(now - self.started, self.interval)
            )
            self.haptics.set_breathing(
                self.motor_mode,
                strength,
                self.direction,
                now + HAPTIC_CONTINUOUS_LEASE_SECONDS,
...
class OutputArbiter:
    def __init__(self, haptics, state):
        ...
        self.breathing = BreathingScheduler(haptics, state)
    ...
    def scheduler(self):
        ...
        if self.state_values["active_mode"] == "purring":
            return self.purr
        if self.state_values["active_mode"] == "snoring":
            return self.snore
    ...
    def switch_mode(self, mode, now):
        ...
        if mode == self.state_values["active_mode"]:
            return False
        was_running = self.state_values["running"]
        self.stop_all(now)
...
class SCD43Monitor:
    """Optional cooperative SCD4x monitor on the STEMMA QT bus."""
    ...
    def __init__(self):
        ...
        try:
            ...
            self.detected = SCD43_ADDRESS in scan_bus(self.i2c)
            if not self.detected:
                print("[SCD43] Not detected at 0x62")
                return
            print("[SCD43] Detected at 0x62; haptics will start silent")
            if adafruit_scd4x is None:
                self.last_error = "adafruit_scd4x library is not installed"
                print("[SCD43] WARNING:", self.last_error)
                return
            self.sensor = adafruit_scd4x.SCD4X(self.i2c)
            self.sensor.start_periodic_measurement()
            self.initialized = True
            self.next_poll = time.monotonic() + SCD43_POLL_SECONDS
            print("[SCD43] Periodic measurement started")
        except Exception as error:
            self.error_count += 1
            self.last_error = str(error)
            print("[SCD43] Initialization warning:", error)

    def update(self, now):
        if not self.initialized or now < self.next_poll:
            return
        self.next_poll = now + SCD43_POLL_SECONDS
        try:
            if not self.sensor.data_ready:
                return
            self.co2 = self.sensor.CO2
            self.temperature = round(self.sensor.temperature, 2)
            self.relative_humidity = round(self.sensor.relative_humidity, 2)
            self.read_count += 1
            self.last_error = ""
        except Exception as error:
            self.error_count += 1
            self.last_error = str(error)
            print("[SCD43] Poll warning:", error)
...
live_state = {
    ...
    "breath_intensity": clamp(int(saved_value(saved_defaults, "bi", "breath_intensity", 55)), 0, 100),
    "breath_rate": clamp(int(saved_value(saved_defaults, "br", "breath_rate", BREATH_RATE_DEFAULT_BPM)), BREATH_RATE_MIN_BPM, BREATH_RATE_MAX_BPM),
    "breath_motor_mode": saved_value(saved_defaults, "bm", "breath_motor_mode", "both"),
    "button_inputs_inverted": truthy(saved_value(saved_defaults, "bx", "button_inputs_inverted", True)),
...
if state["breath_motor_mode"] not in BREATHING_MOTOR_MODES:
    state["breath_motor_mode"] = "both"
...
button_blue_input = None
button_red_input = None
button_controller = None
button_error = ""
if DUAL_BUTTON_ENABLED and BUTTON_POLARITY_PHYSICALLY_VERIFIED:
    try:
        button_blue_input = digitalio.DigitalInOut(board.TX)
        button_blue_input.direction = digitalio.Direction.INPUT
        button_blue_input.pull = None
        button_red_input = digitalio.DigitalInOut(board.RX)
        button_red_input.direction = digitalio.Direction.INPUT
        button_red_input.pull = None
        button_controller = dual_button.DualButtonController(
            button_blue_input,
            button_red_input,
            BUTTON_ACTIVE_LEVEL,
            now=time.monotonic(),
            debounce_seconds=BUTTON_DEBOUNCE_MS / 1000.0,
            double_click_seconds=DOUBLE_CLICK_WINDOW_MS / 1000.0,
            chord_seconds=BUTTON_CHORD_WINDOW_MS / 1000.0,
            long_hold_seconds=BUTTON_LONG_HOLD_MS / 1000.0,
        )
        print("[BUTTON] U025 controls configured on TX=Blue / RX=Red")
        print("[BUTTON] Waiting for both buttons released before arming")
    except Exception as error:
        button_controller = None
        button_error = str(error)
        print("[BUTTON] Initialization warning:", error)
elif DUAL_BUTTON_ENABLED:
    button_error = "polarity pending physical verification"
    print("[BUTTON] Disabled until U025 polarity is physically verified")
...
if state["upstream_enabled"]:
    # CircuitPython's station connect call is blocking. Do not run it during
    # boot: a missing upstream network must never delay the direct AP, HTTP,
    # buttons, sensor polling, or haptic scheduler. The user can explicitly
    # connect from Connection Settings while Running is OFF.
    network_manager.status = "Configured"
    network_manager.error = "Use Connection Settings to connect"
    print("[WIFI] Upstream configured; automatic boot connect deferred")
...
<style>
:root{color-scheme:dark;--red:#ff4d67;--panel:#202126;--muted:#a9abb4;--green:#52d273;--blue:#5e8cff}*{box-sizing:border-box}body{margin:0;background:#101115;color:#fff;font-family:system-ui,sans-serif}main{max-width:480px;margin:auto;padding:16px}.card{background:var(--panel);border-radius:20px;padding:18px;margin-bottom:14px}h1{text-align:center;margin:2px 0 4px;font-size:28px}h2{font-size:14px;letter-spacing:.12em;color:var(--muted);margin:0 0 14px}.connection{text-align:center;color:var(--green);font-size:14px}.segments{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin:16px 0}.segments button.active{background:var(--red)}button,select,input[type=number],input[type=text],input[type=password]{min-height:48px;border:0;border-radius:12px;padding:10px 12px;font-size:16px;background:#383a43;color:#fff;font-weight:650}button.primary{background:var(--red)}button:disabled{opacity:.35}.row{display:flex;align-items:center;justify-content:space-between;gap:10px}.run{font-size:18px}.stat{text-align:center;font-size:48px;font-weight:750;margin:10px 0}.stat small{font-size:17px}.master{border:1px solid #555866;background:#292b33}.master .value{font-size:30px}.label{margin-top:13px}.value{text-align:center;font-size:21px;font-weight:700}input[type=range]{width:100%;height:40px;accent-color:var(--red)}select,input[type=number],input[type=text],input[type=password]{width:100%;background:#30323a}.hidden{display:none}.hint,.status{color:var(--muted);font-size:14px;line-height:1.5}.tests{display:grid;grid-template-columns:1fr 1fr;gap:8px}.timer-buttons{display:grid;grid-template-columns:1fr 1fr;gap:8px}.timer{text-align:center;font-size:27px;font-variant-numeric:tabular-nums;margin:12px}.dot,.notice{color:var(--green)}.bad,.error{color:#ff8798}.settingsButton{width:100%;margin-top:10px}details.instructions summary{cursor:pointer;font-size:17px;font-weight:750}details.instructions table{width:100%;margin-top:14px;border-collapse:collapse;color:var(--muted);font-size:14px;line-height:1.4}details.instructions th,details.instructions td{text-align:left;vertical-align:top;padding:8px 6px;border-bottom:1px solid #484a54}details.instructions th{color:#fff}details.instructions button{width:100%;margin-top:14px}
</style></head><body><main>
<section class="card"><h1>Surrogate Heart</h1><div class="connection">Connection: Direct Wi-Fi</div><div class="segments"><button id="modeHeart">Heart</button><button id="modePurr">Purring</button><button id="modeSnore">Snoring</button><button id="modeBreath">Breathing</button></div><div class="row run"><strong>Running</strong><button id="runToggle">OFF</button></div></section>
<details class="card instructions"><summary>Button Instructions</summary><table><thead><tr><th>Gesture</th><th>Action</th></tr></thead><tbody><tr><td>Both: click</td><td>Next mode: Heart &rarr; Purring &rarr; Snoring &rarr; Breathing</td></tr><tr><td>Top: single-click</td><td>Raise Master Power</td></tr><tr><td>Bottom: single-click</td><td>Lower Master Power</td></tr><tr><td>Top: double-click</td><td>Make the active pattern faster</td></tr><tr><td>Bottom: double-click</td><td>Make the active pattern slower</td></tr><tr><td>Both: hold 3 seconds</td><td>Turn vibrations off; hold again to resume the last mode and settings</td></tr></tbody></table><button id="invertButtons">Input Controls Inverted: ON</button></details>
...
<div id="breathPanel" class="hidden"><section class="card"><h2>BREATHING</h2><div class="stat">BREATHE <small id="breathStat">12/min</small></div><div class="label">Breathing Intensity</div><div id="breathValue" class="value">55%</div><input id="breathIntensity" type="range" min="0" max="100" value="55"><div class="label">Breaths per Minute</div><div id="breathRateValue" class="value">12 breaths/min</div><input id="breathRate" type="range" min="6" max="30" step="1" value="12"><div class="label">Motor Routing</div><select id="breathMotor"><option value="both">Both Motors</option><option value="motor1">Motor 1</option><option value="motor2">Motor 2</option><option value="alternating">Alternating Breaths</option></select><p class="hint">A smooth inhale/exhale envelope. Rate and power changes are adopted at safe breath boundaries.</p></section></div>
...
const renderBase=render;render=data=>{renderBase(data);if(document.activeElement?.type!=="range"){$("breathIntensity").value=data.breathing.intensity;$("breathRate").value=data.breathing.rate_bpm}$("breathMotor").value=data.breathing.motor_mode;const inverted=data.controls.input_controls_inverted;$("invertButtons").textContent="Input Controls Inverted: "+(inverted?"ON":"OFF");$("invertButtons").classList.toggle("primary",inverted);localValues()};
...
slider("master","/api/v1/master","master_power");slider("bpm","/api/v1/heart","bpm");slider("lub","/api/v1/heart","lub_intensity");slider("dub","/api/v1/heart","dub_intensity");slider("delay","/api/v1/heart","lub_dub_delay_ms");slider("purrIntensity","/api/v1/purring","intensity");slider("purrBaseline","/api/v1/purring","baseline");slider("purrRate","/api/v1/purring","rate_ppm");slider("snoreIntensity","/api/v1/snoring","intensity");slider("snoreDuration","/api/v1/snoring","duration");slider("snoreTaper","/api/v1/snoring","taper");slider("snoreGap","/api/v1/snoring","gap");slider("breathIntensity","/api/v1/breathing","intensity");slider("breathRate","/api/v1/breathing","rate_bpm");
...
$("heartMotor").onchange=()=>post("/api/v1/heart",{motor_mode:$("heartMotor").value});$("purrPlayMode").onchange=()=>post("/api/v1/purring",{play_mode:$("purrPlayMode").value});$("purrPattern").onchange=()=>{localValues();post("/api/v1/purring",{motor_pattern:$("purrPattern").value})};$("snoreMotor").onchange=()=>post("/api/v1/snoring",{motor_mode:$("snoreMotor").value});$("breathMotor").onchange=()=>post("/api/v1/breathing",{motor_mode:$("breathMotor").value});
...
$("modeHeart").onclick=()=>post("/api/v1/mode",{mode:"heart"});$("modePurr").onclick=()=>post("/api/v1/mode",{mode:"purring"});$("modeSnore").onclick=()=>post("/api/v1/mode",{mode:"snoring"});$("modeBreath").onclick=()=>post("/api/v1/mode",{mode:"breathing"});$("runToggle").onclick=()=>post("/api/v1/run",{running:!current.running});
$("invertButtons").onclick=()=>post("/api/v1/controls",{inverted:!current.controls.input_controls_inverted});
...
def compact_preferences():
    return {
        ...
        "sg": state["snore_gap"],
        "bi": state["breath_intensity"],
        "br": state["breath_rate"],
        "bm": state["breath_motor_mode"],
        "bx": state["button_inputs_inverted"],
...
def public_state(now):
    return {
        ...
        "breathing": {
            "intensity": state["breath_intensity"],
            "rate_bpm": state["breath_rate"],
            "rate_min_bpm": BREATH_RATE_MIN_BPM,
            "rate_max_bpm": BREATH_RATE_MAX_BPM,
            "motor_mode": state["breath_motor_mode"],
        },
        "snoring_runtime": {
        ...
        "controls": {
            "dual_button_enabled": DUAL_BUTTON_ENABLED,
            # A passive GPIO controller cannot be reliably auto-detected.
            "dual_button_present": None,
            "configured": button_controller is not None,
            "armed": button_controller.armed if button_controller else False,
            "blue_pin": "board.TX",
            "red_pin": "board.RX",
            "polarity_verified": BUTTON_POLARITY_PHYSICALLY_VERIFIED,
            "input_controls_inverted": state["button_inputs_inverted"],
            "error": button_error,
        },
        "environment": scd43.public_status(),
        "network": {
...
def set_master_power(value, now):
    return changed("master_power", clamp(int(value), 0, 100))


def set_heart_bpm(value, now):
    value = clamp(int(value), 30, 240)
    dirty = changed("heart_bpm", value)
    state["heart_delay_ms"] = min(
        state["heart_delay_ms"],
        maximum_delay_ms(state["heart_bpm"]),
    )
    return dirty


def set_snore_gap(value, now):
    return changed("snore_gap", round(clamp(float(value), 0.0, 10.0), 2))


def set_breath_rate(value, now):
    return changed(
        "breath_rate",
        clamp(int(value), BREATH_RATE_MIN_BPM, BREATH_RATE_MAX_BPM),
    )


def set_mode(mode, now):
...
def next_button_mode(now):
    ...
    try:
        index = BUTTON_MODE_SEQUENCE.index(current)
    except ValueError:
        index = -1
    return set_mode(BUTTON_MODE_SEQUENCE[(index + 1) % len(BUTTON_MODE_SEQUENCE)], now)


def adjust_active_mode_speed(faster, now):
    mode = state["active_mode"]
    if mode == "heart":
        step = HEART_BUTTON_BPM_STEP if faster else -HEART_BUTTON_BPM_STEP
        return set_heart_bpm(state["heart_bpm"] + step, now)
    if mode == "snoring":
        duration = state["snore_duration"]
        cycle_interval = duration + state["snore_gap"]
        factor = 1.0 - SNORE_BUTTON_CADENCE_STEP if faster else 1.0 + SNORE_BUTTON_CADENCE_STEP
        return set_snore_gap(cycle_interval * factor - duration, now)
    if mode == "breathing":
        step = BREATH_BUTTON_RATE_STEP_BPM if faster else -BREATH_BUTTON_RATE_STEP_BPM
        return set_breath_rate(state["breath_rate"] + step, now)
    # Purring remains available in the web UI; if selected there, physical
    # double-clicks adjust its existing native rate instead of inventing state.
    step = PURR_BUTTON_RATE_STEP_PPM if faster else -PURR_BUTTON_RATE_STEP_PPM
    return changed(
        "purr_rate",
        clamp(state["purr_rate"] + step, PURR_RATE_MIN_PPM, PURR_RATE_MAX_PPM),
    )


def effective_button_event(event):
    if not state["button_inputs_inverted"]:
        return event
    if event == dual_button.BLUE_SINGLE:
        return dual_button.RED_SINGLE
    if event == dual_button.RED_SINGLE:
        return dual_button.BLUE_SINGLE
    if event == dual_button.BLUE_DOUBLE:
        return dual_button.RED_DOUBLE
    if event == dual_button.RED_DOUBLE:
        return dual_button.BLUE_DOUBLE
    return event


def handle_button_event(event, now):
    """Map semantic gestures to the same live_state setters used by HTTP."""
    event = effective_button_event(event)
    if event == dual_button.BLUE_SINGLE:
        return set_master_power(state["master_power"] + MASTER_STEP, now)
    if event == dual_button.RED_SINGLE:
        return set_master_power(state["master_power"] - MASTER_STEP, now)
    if event == dual_button.BLUE_DOUBLE:
        return adjust_active_mode_speed(True, now)
    if event == dual_button.RED_DOUBLE:
        return adjust_active_mode_speed(False, now)
    if event == dual_button.BOTH_SHORT:
        return next_button_mode(now)
    if event == dual_button.BOTH_LONG:
        set_running(not state["running"], now)
        return True
    return False
...
def apply_master(params, now):
    if params.get("master_power") is not None:
        set_master_power(params.get("master_power"), now)
...
def apply_heart(params, now):
    ...
    if selected_bpm is not None:
        dirty = set_heart_bpm(selected_bpm, now) or dirty
    if params.get("bpm") is not None:
        dirty = set_heart_bpm(params.get("bpm"), now) or dirty
...
def apply_snore(params, now):
    ...
    if params.get("gap") is not None:
        dirty = set_snore_gap(params.get("gap"), now) or dirty
    return dirty


def apply_breathing(params, now):
    dirty = False
    if params.get("intensity") is not None:
        dirty = changed("breath_intensity", clamp(int(params.get("intensity")), 0, 100)) or dirty
    requested_rate = params.get("rate_bpm")
    if requested_rate is None:
        requested_rate = params.get("breath_rate")
    if requested_rate is not None:
        dirty = set_breath_rate(requested_rate, now) or dirty
    if params.get("motor_mode") in BREATHING_MOTOR_MODES:
        dirty = changed("breath_motor_mode", params.get("motor_mode")) or dirty
    return dirty


def apply_controls(params, now):
    if params.get("inverted") is None:
        return False
    return changed("button_inputs_inverted", truthy(params.get("inverted")))
...
@server.route("/api/v1/breathing", POST)
def api_breathing(request: Request):
    data = request_data(request)
    motor_mode = data.get("motor_mode")
    if motor_mode is not None and motor_mode not in BREATHING_MOTOR_MODES:
        return json_response(request, {
            "error": "invalid motor_mode",
            "valid": BREATHING_MOTOR_MODES,
        })
    apply_breathing(data, time.monotonic())
    return json_response(request, public_state(time.monotonic()))


@server.route("/api/v1/controls", POST)
def api_controls(request: Request):
    apply_controls(request_data(request), time.monotonic())
    return json_response(request, public_state(time.monotonic()))
...
def compatibility_heartbeat(request: Request):
    ...
    if params.get("effect_mode") in ACTIVE_MODES and params.get("effect_mode") != state["active_mode"]:
        set_mode(params.get("effect_mode"), now)
...
def legacy_toggle(request: Request):
    set_running(not state["running"], time.monotonic())
...
if (
    ...
    if scd43.detected:
        arbiter.stop_all(startup_complete)
        print("[MODE] {} / OFF (SCD43 startup-silence rule)".format(state["active_mode"]))
        startup_report_pending = False
    else:
        set_running(True, startup_complete)
...
try:
    ...
    while True:
        ...
        if button_controller is not None:
            try:
                button_event = button_controller.update(loop_now)
                if button_event is not None:
                    handle_button_event(button_event, loop_now)
                    effective_event = effective_button_event(button_event)
                    print(
                        "[BUTTON] {} mode={} running={} master={} heart_bpm={} "
                        "snore_gap={} breath_rate={}".format(
                            effective_event,
                            state["active_mode"],
                            state["running"],
                            state["master_power"],
                ...
                button_error = str(error)
                button_controller = None
                print("[BUTTON] Disabled after input error:", error)
        scd43.update(loop_now)
        try:
            server.poll()
        except Exception as error:
